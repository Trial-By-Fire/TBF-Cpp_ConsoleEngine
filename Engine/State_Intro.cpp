...
{
	Renderer_WriteToLog((WideChar*)L"Intro State: Loaded");
...
{
	Renderer_WriteToLog((WideChar*)L"Intro State: Unloaded");
}
...
void IntroState_Update(void)
{
	...
	Renderer_WriteToPersistentSection(4, (WideChar*)L"Intro Time Elapsed: %.7Lf", IntroTimer.Elapsed);

	if (Timer_Ended(&Timer_TillTitle))
	{
		if (LogTitle)
		{
			Renderer_WriteToLog((WideChar*)L"Title should show up now (Grey).");
		...
		if (Log_ChangeToWhite_Title &&  Timer_Ended(&Timer_TillTitle_ToWhite))
		{
			...
			Renderer_WriteToLog((WideChar*)L"Title: White attribute set.");
		...
		{
			Renderer_WriteToLog((WideChar*)L"Version should show up now.");
		...
		if (RenderVersion)
		{
			...
			if (Log_ChangeToWhite_Version && Timer_Ended(&Timer_TillVersion_ToWhite))
			{
				...
				Renderer_WriteToLog((WideChar*)L"Engine Version: White attribute set.");
	...
	if (Timer_Ended(&Timer_TillIntroFadeToGrey))
	{
		...
		if (Log_FadeToGrey)
		{
			Renderer_WriteToLog((WideChar*)L"Title should fade to gray now.");
		...
		{
			Renderer_WriteToLog((WideChar*)L"Title should fade out now.");
	...
	{
		Renderer_WriteToLog((WideChar*)L"Intro Ends now.");
