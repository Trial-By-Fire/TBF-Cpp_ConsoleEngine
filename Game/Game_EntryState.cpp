...
{
	Renderer_WriteToLog((WideChar*)L"UI Start Selected");
...
void Game_EntryState_OnKeyArrowUp(EInputState _state)
{
	switch (_state)
	{
		case EInput_Pressed:
		{
			Renderer_WriteToLog((WideChar*)L"EntryState: On Key Up");
...
void Game_EntryState_OnKeyArrowDown(EInputState _state)
{
	switch (_state)
	{
		case EInput_Pressed:
		{
			Renderer_WriteToLog((WideChar*)L"EntryState: On Key Down");
...
void Game_EntryState_OnKeyEnter(EInputState _state)
{
	switch (_state)
	{
		case EInput_Pressed:
		{
			Renderer_WriteToLog((WideChar*)L"EntryState: On Key Enter");
...
void MainMenu_Load(void)
{
	...
	if (Log_Load)
	{
		Renderer_WriteToLog((WideChar*)L"Loaded: Main Menu");
...
void MainMenu_Unload(void)
{
	...
	if (Log_Unload)
	{
		Renderer_WriteToLog((WideChar*)L"Unload: Main Menu");
